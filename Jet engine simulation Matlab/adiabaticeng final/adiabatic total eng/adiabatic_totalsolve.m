...
SR=1.5;%Air/fuel ratio divided by stoichiometric air/fuel ratio
...
%Final results
disp('Number of moles air/unit fuel');
disp(SR*57.6);
disp('Moles resultant gas/mol fuel');
disp(nmol);
disp('Tfin of compressor stage');
disp(Tfin);
disp('Enthalpy of compressor stage KJ/mol fuel');
disp(htot);
disp('Tout of combustion stage');
disp(Tcomb2);
disp('Energy lost during combustuion KJ/mol fuel');
disp(Eheatloss);
disp('Temperature celing vs calculated difference');
disp(TempDiff);
disp('Tout of nozzle stage');
disp(Te2);
disp('Enthalpy of nozzle stage KJ/mol fuel');
disp(hetot);
disp('Net energy ratio Production/consumption');
disp(-hetot/htot);
disp('Absolute Efficiency without bypass');
disp((1000*hetot+1000*htot)/Enthcombust);
disp('Molar Kinetic Energy');
disp(-KE/nmol);
disp('Exhaust velocity m/s without bypass');
disp((-KE/(.0143*nmol))^.5);


xlswrite('jetenginestats.xlsx',{'Inlet P in bar','Inlet T in K','Compression Pressure','Outlet Pressure'}); 
xlswrite('jetenginestats.xlsx',[P1 T1 P2 Pdisch],1,'A2');
xlswrite('jetenginestats.xlsx',{'Moles air inlet/mol fuel' 'Moles resultant gas/mol fuel' 'Tfin of compressor stage' 'Enthalpy of compressor stage KJ/mol fuel','Tout of combustion stage','Energy lost during combustuion KJ/mol fuel','Temperature celing vs calculated difference','Tout of nozzle stage','Enthalpy of nozzle stage KJ/mol fuel','Net energy ratio Production/consumption','Absolute Efficiency without bypass','Molar Kinetic Energy','Exhaust velocity m/s without bypass'},1,'A3');
xlswrite('jetenginestats.xlsx',[(SR*57.6) nmol Tfin htot Tcomb2 Eheatloss TempDiff Te2 hetot -hetot/htot (1000*hetot+1000*htot)/Enthcombust -KE/nmol (-KE/(.0143*nmol))^.5],1,'A4');